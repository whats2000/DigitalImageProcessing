...
def create_hw2_panel(app: 'ImageProcessorApp', container: tk.Frame):
    ...
    _setup_dip_buttons_frame(app, panel)
    return panel
...
"""
4. Consider the image below (DIP_image.tif). The image was processed by the following
steps:
(1) multiplying the image by (-1)^(x+y) ;
(2) computing the DFT;
...
(a) What would the processed image look like? Show the processed images from each step.
(b) Explain mathematically why it appears as it does.
"""


def _setup_dip_buttons_frame(app: 'ImageProcessorApp', parent_frame: tk.Frame):
    """
    Set up the frame for applying filters to the test pattern
    """
    # Create labels for the mask buttons
    ...
    button_frame = tk.Frame(parent_frame, bg=MAIN_THEME, pady=10)
    button_frame.pack(anchor="w", pady=5)
    step_1_button = tk.Button(
        button_frame,
        text="Multiply by (-1)^(x+y)",
        command=app.operations.multiply_by_neg_1
    )
    step_2_button = tk.Button(
        button_frame,
        text="Compute DFT",
        command=app.operations.compute_dft
    )
    step_1_button.pack(side=tk.LEFT)
    step_2_button.pack(side=tk.LEFT, padx=10)
    button_frame_2 = tk.Frame(parent_frame, bg=MAIN_THEME, pady=10)
    button_frame_2.pack(anchor="w", pady=5)
    step_3_button = tk.Button(
        button_frame_2,
        text="Complex Conjugate",
        command=app.operations.take_conjugate
    )
    step_4_button = tk.Button(
        button_frame_2,
        text="Compute InvDFT",
        command=app.operations.compute_inverse_dft
    )
    step_3_button.pack(side=tk.LEFT)
    step_4_button.pack(side=tk.LEFT, padx=10)
    button_frame_3 = tk.Frame(parent_frame, bg=MAIN_THEME, pady=10)
    button_frame_3.pack(anchor="w", pady=5)
    step_5_button = tk.Button(
        button_frame_3,
        text="Multiply by (-1)^(x+y)",
        command=app.operations.multiply_by_neg_1_final
    )
    step_5_button.pack(side=tk.LEFT)
